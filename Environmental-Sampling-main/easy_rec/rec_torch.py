...
class SequentialCollator:
    ...
    def pair_input_output(self, data, seq_lens):
        ...
        ######################## added ########################
        # batch_size, sequence_length = data['in_sid'].shape
        # for j in range(batch_size):
        #     # Find the index of the first non-zero element
        #     first_non_zero = 0
        #     while first_non_zero < sequence_length and int(data['in_sid'][j][first_non_zero]) == 0:
        #         first_non_zero += 1
            
        #     if first_non_zero == sequence_length:
        #         # All zeros, nothing to do
        #         continue
            
        #     # Extract the useful (non-zero) elements
        #     useful_elements = data['in_sid'][j][first_non_zero:].clone()
        #     useful_length = len(useful_elements)
            
        #     if useful_length == 0:
        #         continue
            
        #     # Create a new sequence with repeated patterns
        #     new_sequence = torch.zeros_like(data['in_sid'][j])
        #     current_pos = 0
            
        #     while current_pos < sequence_length:
        #         # Calculate how many elements we can fit
        #         remaining_space = sequence_length - current_pos
        #         elements_to_copy = min(useful_length, remaining_space)
                
        #         if elements_to_copy <= 0:
        #             break
                    
        #         # Copy elements
        #         new_sequence[current_pos:current_pos+elements_to_copy] = useful_elements[:elements_to_copy]
        #         current_pos += elements_to_copy
                
        #         # Add separator if there's room
        #         if current_pos < sequence_length:
        #             new_sequence[current_pos] = 0
        #             current_pos += 1
            
        #     # Update the original data with our new sequence
        #     data['in_sid'][j] = new_sequence
